...
#define COL_SIZE	20
    ...
    TEST_METHOD (APIS_932_DEFAULT_AUTO_COMMIT_ON)
    {
      ...
      retcode = SQLDriverConnect (hDbc, NULL,
				  L"DRIVER=CUBRID Driver Unicode;server=test-db-server;port=33000;uid=public;pwd=;db_name=demodb;charset=utf-8;", SQL_NTS,
				  NULL, 0, NULL, SQL_DRIVER_NOPROMPT);
      Assert::AreNotEqual ((int)retcode, SQL_ERROR);

      retcode = SQLGetConnectAttr (hDbc, SQL_ATTR_AUTOCOMMIT, &autocommit, 0, NULL);
      wsprintf (msg, L"case 1 (url off by default): ++ AUTOCOMMIT = %s",
		autocommit ? L"SQL_AUTOCOMMIT_ON (1)" : L"SQL_AUTOCOMMIT_OFF (0)");
      Logger::WriteMessage (msg);
      ...
      retcode = SQLAllocConnect (hEnv, &hDbc);
      retcode = SQLDriverConnect (hDbc, NULL,
				  L"DRIVER=CUBRID Driver Unicode;server=test-db-server;port=33000;uid=public;pwd=;db_name=demodb;charset=utf-8;autocommit=on",
				  SQL_NTS, NULL, 0, NULL, SQL_DRIVER_NOPROMPT);
      Assert::AreNotEqual ((int)retcode, SQL_ERROR);

      retcode = SQLGetConnectAttr (hDbc, SQL_ATTR_AUTOCOMMIT, &autocommit, 0, NULL);
      wsprintf (msg, L"case 2 (url on): ++ AUTOCOMMIT = %s",
		autocommit ? L"SQL_AUTOCOMMIT_ON (1)" : L"SQL_AUTOCOMMIT_OFF (0)");
      Logger::WriteMessage (msg);
      Assert::AreEqual ((int)autocommit, AUTO_COMMIT_ON);
      ...
      retcode = SQLAllocConnect (hEnv, &hDbc);
      retcode = SQLDriverConnect (hDbc, NULL,
				  L"DRIVER=CUBRID Driver Unicode;server=test-db-server;port=33000;uid=public;pwd=;db_name=demodb;charset=utf-8;autocommit=off",
				  SQL_NTS, NULL, 0, NULL, SQL_DRIVER_NOPROMPT);
      Assert::AreNotEqual ((int)retcode, SQL_ERROR);

      retcode = SQLGetConnectAttr (hDbc, SQL_ATTR_AUTOCOMMIT, &autocommit, 0, NULL);
      wsprintf (msg, L"case 3 (url off): ++ AUTOCOMMIT = %s",
		autocommit ? L"SQL_AUTOCOMMIT_ON (1)" : L"SQL_AUTOCOMMIT_OFF (0)");
      Logger::WriteMessage (msg);
      ...
      retcode = SQLGetConnectAttr (hDbc, SQL_ATTR_AUTOCOMMIT, &autocommit, 0, NULL);
      wsprintf (msg, L"case 4 (DSN off): ++ AUTOCOMMIT = %s",
		autocommit ? L"SQL_AUTOCOMMIT_ON (1)" : L"SQL_AUTOCOMMIT_OFF (0)");
      Logger::WriteMessage (msg);
      Assert::AreEqual ((int)autocommit, AUTO_COMMIT_OFF);
      ...
      retcode = SQLGetConnectAttr (hDbc, SQL_ATTR_AUTOCOMMIT, &autocommit, 0, NULL);
      wsprintf (msg, L"case 5 (DSN on): ++ AUTOCOMMIT = %s",
		autocommit ? L"SQL_AUTOCOMMIT_ON (1)" : L"SQL_AUTOCOMMIT_OFF (0)");
      Logger::WriteMessage (msg);
      Assert::AreEqual ((int)autocommit, AUTO_COMMIT_ON);
      ...
      retcode = SQLGetConnectAttr (hDbc, SQL_ATTR_AUTOCOMMIT, &autocommit, 0, NULL);
      wsprintf (msg, L"case 6  (DSN on by default):++ AUTOCOMMIT = %s",
		autocommit ? L"SQL_AUTOCOMMIT_ON (1)" : L"SQL_AUTOCOMMIT_OFF (0)");
      Logger::WriteMessage (msg);
      Assert::AreEqual ((int)autocommit, AUTO_COMMIT_ON);
    ...
    TEST_METHOD (APIS_939_TABLE_COLUMN_COMMENT)
    {
      ...
      SQLWCHAR		*qry_alt_column_comment =
	      L"ALTER TABLE olympic COMMENT ON COLUMN host_year = 'The year the Olympics were held';";
    ...
    TEST_METHOD (APIS_943_SQLCOLUMNSW)
    {
      ...
      retcode = SQLSetEnvAttr (hEnv, SQL_ATTR_ODBC_VERSION, (void *)SQL_OV_ODBC3, 0);
      retcode = SQLAllocConnect (hEnv, &hDbc);
      retcode = SQLDriverConnectW (hDbc, NULL,
				   L"DRIVER=CUBRID Driver Unicode;server=test-db-server;port=33000;uid=public;pwd=;db_name=demodb;charset=utf-8;", SQL_NTS,
				   NULL, 0, NULL, SQL_DRIVER_NOPROMPT);
      Assert::AreNotEqual ((int)retcode, SQL_ERROR);
      retcode = SQLAllocHandle (SQL_HANDLE_STMT, hDbc, &hstmt);
    ...
    TEST_METHOD (APIS_942_OMIT_SCHEMA)
    {
      ...
      retcode = SQLAllocConnect (hEnv, &hDbc);
      retcode = SQLDriverConnect (hDbc, NULL,
				  L"DRIVER=CUBRID Driver Unicode;server=test-db-server;port=33000;uid=dba;pwd=;db_name=demodb;charset=utf-8", SQL_NTS,
				  NULL, 0, NULL, SQL_DRIVER_NOPROMPT);
      Assert::AreNotEqual ((int)retcode, SQL_ERROR);
      retcode = SQLAllocHandle (SQL_HANDLE_STMT, hDbc, &hstmt);
      ...
      retcode = SQLAllocHandle (SQL_HANDLE_STMT, hDbc, &hstmt);
      retcode = SQLDriverConnect (hDbc, NULL,
				  L"DRIVER=CUBRID Driver Unicode;server=test-db-server;port=33000;uid=user1;pwd=1234;db_name=demodb;charset=utf-8;omit_schema=yes",
				  SQL_NTS, NULL, 0, NULL, SQL_DRIVER_NOPROMPT);
      Assert::AreNotEqual ((int)retcode, SQL_ERROR);
      retcode = SQLAllocHandle (SQL_HANDLE_STMT, hDbc, &hstmt);
      ...
      while (SQL_SUCCEEDED (retcode = SQLFetch (hstmt)))
	{
      ...
      retcode = SQLAllocHandle (SQL_HANDLE_STMT, hDbc, &hstmt);
      retcode = SQLDriverConnect (hDbc, NULL,
				  L"DRIVER=CUBRID Driver Unicode;server=test-db-server;port=33000;uid=public;pwd=;db_name=demodb;charset=utf-8;omit_schema=yes",
				  SQL_NTS, NULL, 0, NULL, SQL_DRIVER_NOPROMPT);
      Assert::AreNotEqual ((int)retcode, SQL_ERROR);
      retcode = SQLAllocHandle (SQL_HANDLE_STMT, hDbc, &hstmt);
...
#define MAX_COL_NAME_LEN  256
	...
	{
	  retcode = SQLDescribeColW (hStmt, i, col_name, sizeof (col_name) / sizeof (SQLWCHAR), &name_len, &data_type, &col_size,
      ...
      for (SQLUSMALLINT i = 1; i <= numCols; i++)
	{
	  retcode = SQLDescribeColW (hStmt, i, NULL, sizeof (col_name) / sizeof (SQLWCHAR), &name_len, &data_type, &col_size,
				     &decimal_digits, &nullable);
	  if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
	    {
	      Assert::Fail (L"Error getting column information");
